...
int testIoBuffer(){
	...
	auto eos = map.getSummary(3600, "eos");
	if (eos.has_value()){
		IoBuffer::Summary eosBuf;

		std::cout << "summary\n" << eos << std::endl;
		if (eos.has_value())
			eos->Serialize(eosBuf);
		std::cout << "protobof\n" << eosBuf << std::endl;
		eos->Deserialize(eosBuf);
		std::cout << "summary deserialize\n" << eos << std::endl;
		std::string output;
		eosBuf.SerializeToString(&output);
		std::cout << "protobuof serialize\n" << output << std::endl;
	} else
		return -1;
	return 0;
}
